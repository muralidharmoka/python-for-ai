...
def qa_agent(state: CodeState):
    prompt = f"""
    You are a senior strict QA agent for Nodejs.
    Evaluate the following Nodejs code based on the following criteria:

    1. Code correctness: Does the code meet the user's request and function as intended?
    2. Structure and organization: Is the code well-structured and organized?
    3. Production best practices: Does the code follow best practices for production-ready code?
    4. Error handling: Does the code include proper error handling?
    5. Use of variables and functions: Are naming and usage clear and appropriate?

    Explain clearly what is wrong with the code if the rating is less than 7.
    ...
    response = llm_qa.invoke(prompt).content
